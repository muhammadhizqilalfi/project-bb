    ...
    <aside class="w-64 bg-[#0E1B2E] text-white flex flex-col shrink-0 min-h-screen z-20 shadow-xl">
        ...
        <nav class="flex-1 px-3.5 py-6 space-y-1.5 overflow-y-auto">
            ...
            <div>
                <button
                    type="button"
                    @click="isFormDropdownOpen = !isFormDropdownOpen"
...
const props = defineProps({
    userRole: {
        type: String,
        default: 'karyawan' // Pilihan: 'admin' atau 'karyawan' / 'user'
    },
    activeMenu: {
        type: String,
        default: 'BERANDA'
...
const emit = defineEmits(['update:activeMenu', 'menu-select']);

const isFormDropdownOpen = ref(true);

// Mengubah role ke lowercase agar tidak ada bug perbedaan kapital huruf
const normalizedRole = computed(() => props.userRole?.toLowerCase() || 'karyawan');

const selectMenu = (menuName) => {
    emit('update:activeMenu', menuName);
    emit('menu-select', menuName);
};
</script>