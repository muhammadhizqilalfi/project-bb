        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="lg:col-span-5 bg-white rounded-xl shadow-xs border border-slate-200/80 p-6 space-y-5 h-fit">
            <div class="flex items-center gap-3 pb-3 border-b border-slate-100">
              ...
              <h2 class="text-xs font-bold text-slate-900 uppercase tracking-wider">1. PERKARA & REGISTER</h2>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">NO. REG SITAAN <span class="text-red-500">*</span></label>
            ...
            <div class="pt-3 border-t border-slate-100 space-y-4">
              <div class="flex items-center gap-2 text-slate-800 font-bold text-xs">
                <FileCheck2 class="w-4 h-4 text-slate-600" />
                <span>2. PUTUSAN & EKSEKUSI HAKIM</span>
              </div>

              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label class="block text-[10px] font-bold text-slate-600 uppercase mb-1">NO. KEP (PN/PT/MA) <span class="text-red-500">*</span></label>
                  <input v-model="formCase.noKepPengadilan" type="text" required placeholder="No. Putusan" class="w-full bg-[#F4F6F8] border border-transparent rounded-lg px-3 py-2 text-xs text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-[#FFD000]" />
                </div>
                <div>
                  <label class="block text-[10px] font-bold text-slate-600 uppercase mb-1">TGL. KEP (PN/PT/MA) <span class="text-red-500">*</span></label>
                  <input v-model="formCase.tglKepPengadilan" type="date" required class="w-full bg-[#F4F6F8] border border-transparent rounded-lg px-3 py-2 text-xs text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-[#FFD000]" />
                </div>
              </div>

              <div>
                <label class="block text-[10px] font-bold text-slate-600 uppercase mb-1">AMAR PUTUSAN <span class="text-red-500">*</span></label>
                <textarea v-model="formCase.amarPutusan" rows="2" required placeholder="Contoh: Dirampas untuk dimusnahkan..." class="w-full bg-[#F4F6F8] border border-transparent rounded-lg p-2.5 text-xs text-slate-800 outline-none focus:bg-white focus:ring-2 focus:ring-[#FFD000] resize-none"></textarea>
              </div>

              <div>
                <label class="block text-[10px] font-bold text-slate-600 uppercase mb-1">TGL PELAKSANAAN PUTUSAN</label>
            ...
            >
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input v-model="bb.jenisBarangBukti" type="text" required placeholder="e.g. Narkotika / Senjata Api" class="w-full bg-[#F4F6F8] border border-transparent rounded-lg px-3.5 py-2.5 text-xs text-slate-800 outline-none focus:bg-white focus:border-slate-300 focus:ring-2 focus:ring-[#FFD000] transition-all" />
                </div>
                <div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input v-model="bb.jumlahSatuan" type="number" step="any" required placeholder="0.00" class="w-full bg-[#F4F6F8] border border-transparent rounded-lg px-3.5 py-2.5 text-xs text-slate-800 outline-none focus:bg-white focus:border-slate-300 focus:ring-2 focus:ring-[#FFD000] transition-all" />
                </div>
                <div>
                  ...
                  <select v-model="bb.jenisSatuan" required class="w-full bg-[#F4F6F8] border border-transparent rounded-lg px-3.5 py-2.5 text-xs text-slate-800 outline-none focus:bg-white focus:border-slate-300 focus:ring-2 focus:ring-[#FFD000] transition-all">
              ...
              <div>
                <label class="block text-[11px] font-bold text-slate-600 uppercase tracking-wider mb-1.5">MACAM JENIS KADAR BARANG BUKTI <span class="text-red-500">*</span></label>
                <textarea v-model="bb.macamJenisKadar" rows="2" required placeholder="Spesifikasi rinci, kadar kemurnian, nomor mesin..." class="w-full bg-[#F4F6F8] border border-transparent rounded-lg p-3 text-xs text-slate-800 outline-none focus:bg-white focus:border-slate-300 focus:ring-2 focus:ring-[#FFD000] transition-all resize-none"></textarea>
              </div>
            </div>
          </div>
            ...
            <table class="w-full text-left border-collapse">
              <thead class="bg-slate-800 text-white text-[10px] uppercase tracking-wider">
                <tr>
                  ...
                  <th class="p-3 border border-slate-700">No. Reg Sitaan & Tgl Terima</th>
                  <th class="p-3 border border-slate-700">Pasal Didakwakan</th>
              ...
              <tbody class="text-xs text-slate-700">
                ...
                >
                  <td class="p-3 border border-slate-300 align-top">
                    ...
                    <div class="font-bold text-slate-900">{{ c.kejaksaan || '-' }}</div>
                    <div class="text-[10px] text-slate-500 mt-0.5">{{ c.kategoriTindakPidana || '-' }}</div>
                  </td>
                  <td class="p-3 border border-slate-300 align-top">
                    <div><span class="font-semibold text-slate-900">Reg:</span> {{ c.noRegBendaSitaan || '-' }}</div>
                    <div class="mt-1"><span class="font-semibold text-slate-900">Tgl Terima:</span> {{ c.tglPenerimaan || '-' }}</div>
                  </td>
                  <td class="p-3 border border-slate-300 align-top font-medium text-slate-800">
                    {{ c.pasalDidakwakan || '-' }}
                  </td>
                  <td class="p-3 border border-slate-300 align-top">
                    <div class="space-y-2">
                      ...
                      >
                        <div class="font-bold text-slate-900">{{ idx + 1 }}. {{ bb.jenisBarangBukti || 'Jenis BB' }}</div>
                        <div class="text-[11px] text-slate-600 mt-0.5">{{ bb.macamJenisKadar || '-' }}</div>
                        <div class="mt-1 flex items-center justify-between text-[11px]">
                          <span class="font-bold text-blue-700">Total: {{ bb.jumlahSatuan || 0 }} {{ bb.jenisSatuan }}</span>
                  ...
                  <td class="p-3 border border-slate-300 align-top">
                    <div class="font-semibold text-slate-900 line-clamp-2" :title="c.amarPutusan">{{ c.amarPutusan || '-' }}</div>
                    <div class="mt-2 text-[10px] text-slate-500">
                      <div><span class="font-bold">No KEP:</span> {{ c.noKepPengadilan || '-' }}</div>
                      <div><span class="font-bold">Pelaksanaan:</span> {{ c.tglPelaksanaanPutusan || 'Belum' }}</div>
                    </div>
                  </td>
                </tr>
                ...
                <tr class="bg-amber-50/50 border-2 border-amber-300">
                  ...
                  <td class="p-3 border border-slate-300 align-top">
                    ...
                    <div class="mt-1"><span class="font-semibold text-slate-900">Tgl Terima:</span> {{ formCase.tglPenerimaan || '-' }}</div>
                  </td>
                  <td class="p-3 border border-slate-300 align-top font-medium text-slate-800">
